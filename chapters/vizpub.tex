...
\item[Ranking Component] \hfill \\
    ...
    ranking component also include a ranking table, as seen in
    Figure~\ref{fig:ranking_table}, where the user may
    sort nodes based on the specified attribute value, which is
    useful for quickly finding the nodes with maximum and
    ...
    graph via the timeline component, i.e.\ the node labels and color
    will update with every step.
...
\item[Filter Component] \hfill \\
    ...
    \begin{figure}[h!]
        ...
        \caption{The Filter Component (to the right) enables users to strip away
            ...
            display the sub-overlay for a specific topic}
        \label{fig:ranking_table}
    \end{figure}
...
\item[Data Laboratory Component] \hfill \\
    ...
    \begin{figure}[h!]
        ...
        of metrics as well as the capability of exporting data for
    ...
    for that particular node or edge. In Figure~\ref{fig:data_lab}, the
    data lab is used to inspect the duplicate message received attribute
    for nodes. Columns may be added or removed by the user. The data
    laboratory also provides functionality for manipulating columns such
    as merging two columns or creating new columns based on data from
    selected columns. Attribute data in columns that are static (i.e.\
    has no lower or upper time interval bound associated with them) can
    be converted to dynamic through this component. Also, resizing or
    coloring all edges or nodes is possible through the laboratory by
    selecting all rows and right-clicking. In addition to this, the data
    laboratory also enables the user to export the data to file for
    further statistical analysis.
