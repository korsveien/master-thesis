...
Our tool supports two types of visualizations, the first is a
visualization of the overlay structure, and how it evolves over time.
The second type of visualization is a hop-by-hop animation of
...
by visually comparing the two overlays in Figure~\ref{fig:struct}, we can
observe that the overlay of PolderCast seem to include a lot more edges,
as well as being more dense than the overlay in Scribe. The edges in the
PolderCast overlay are so numerous they almost seem like a mist of white
noise. As observed in Table~\ref{tab:struct_metrics}, both overlays consists of the same number of nodes,
but PolderCast has almost ten times the number of edges. This should mean
PolderCast also has a much higher average degree as well as a higher
graph density. Executing these metrics using the Statistics Component reveal that PolderCast indeed have
a much higher degree, almost ten times higher than Scribe. The \emph{graph
density} is also much higher in PolderCast. Graph density is measured as
how far away the graph is to being complete, where 1 is a complete
graph. The lower density of Scribe is a natural consequence of the lower
number of edges. The difference in edge count is due to the average node
degree having a lower upper bound in Scribe, as described in
Chapter~\ref{ch:design-challenges}. More specifically, the average node
degree in Scribe is bound by the logarithm of the number of nodes in the
overlay $O(log |\mathcal V|)$, while in PolderCast it is in the order of the number
of subscriptions $O(|\mathcal T_v|)$.
...
\begin{figure}
    ...
    \subfigure[PolderCast]{%
        \includegraphics[scale=0.44]{figures/polder_struct}
    }
    \subfigure[Scribe]{%
        \includegraphics[scale=0.44]{figures/scribe_struct}
    }
...
Figure~\ref{fig:scribe_degree_distr} describes the undirected degree distribution of
PolderCast and Scribe. Both distributions are skewed to the right.
However, the range of values are more tightly bound in Scribe. As
mentioned, PolderCast has a average node degree proportional to the
number of subscriptions, which results in a larger range of values than
what is seen in Scribe. This might give Scribe an advantage in terms of
node degree if there are many nodes in the system with a high number of
subscriptions.
...
\begin{figure}
    ...
    \thisfloatpagestyle{empty}
    \subfigure[PolderCast]{%
        \includegraphics[scale=0.49]{figures/polder_degree}
        \label{fig:polder_degree}
    }
    \subfigure[Scribe]{%
        \includegraphics[scale=0.49]{figures/scribe_degree}
